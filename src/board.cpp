...
std::pair<uint8_t, uint32_t> Board::get_field(uint32_t x, uint32_t y)
{
...
uint8_t Board::get_value(uint32_t x, uint32_t y)
{
    return get_field_ref(x, y) & value_mask;
}
...
uint32_t Board::get_flags(uint32_t x, uint32_t y)
{
    return get_field_ref(x, y) & flag_mask;
}
